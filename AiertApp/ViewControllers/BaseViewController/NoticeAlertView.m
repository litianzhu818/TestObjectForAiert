...
static const char * const kNoticeAlertViewButtonBlockKey = "NoticeAlertViewButtonBlock";

static const CGFloat NoticeAlertViewDefaultWidth = 276;
static const CGFloat NoticeAlertViewMinimumHeight = 132;
static const CGFloat NoticeAlertViewDefaultButtonHeight = 40;
...
typedef void (^NoticeAnimationCompletionBlock)(BOOL); // Internal.
...
{
	CGRect frame = CGRectMake(0, 0, NoticeAlertViewDefaultWidth, NoticeAlertViewMinimumHeight);
	...
	if((self = [super initWithFrame:frame]))
	{
		...
		_presentationStyle = NoticeAlertViewPresentationStyleDefault;
		_dismissalStyle = NoticeAlertViewDismissalStyleDefault;
...
- (id)initWithTitle:(NSString *)title message:(NSString *)message cancelButtonTitle:(NSString *)cancelButtonTitle otherButtonTitle:(NSString *)otherButtonTitle
{
	...
	if((self = [super initWithFrame:frame]))
	{
		...
		_presentationStyle = NoticeAlertViewPresentationStyleDefault;
		_dismissalStyle = NoticeAlertViewDismissalStyleTumble;
...
- (void)dealloc
{
	for(id button in _otherButtons)
		objc_setAssociatedObject(button, kNoticeAlertViewButtonBlockKey, nil, OBJC_ASSOCIATION_RETAIN);

	if(_cancelButton)
		objc_setAssociatedObject(_cancelButton, kNoticeAlertViewButtonBlockKey, nil, OBJC_ASSOCIATION_RETAIN);
	
	if(_destructiveButton)
		objc_setAssociatedObject(_destructiveButton, kNoticeAlertViewButtonBlockKey, nil, OBJC_ASSOCIATION_RETAIN);
...
- (UIButton *)buttonWithTitle:(NSString *)aTitle associatedBlock:(NoticeAlertViewButtonBlock)block {
    UIButton *button = [[UIButton alloc] initWithFrame:CGRectZero];
	
	[button setTitle:aTitle forState:UIControlStateNormal];
	[button addTarget:self action:@selector(buttonWasPressed:) forControlEvents:UIControlEventTouchUpInside];
	objc_setAssociatedObject(button, kNoticeAlertViewButtonBlockKey, block, OBJC_ASSOCIATION_RETAIN);
	return button;
}

- (void)addButtonWithTitle:(NSString *)aTitle block:(NoticeAlertViewButtonBlock)block {
...
- (void)setDestructiveButtonTitle:(NSString *)aTitle block:(NoticeAlertViewButtonBlock)block {
	self.destructiveButton = [self buttonWithTitle:aTitle associatedBlock:block];
	[self addSubview:self.destructiveButton];
}

- (void)setCancelButtonTitle:(NSString *)aTitle block:(NoticeAlertViewButtonBlock)block {
...
- (void)showWithStyle:(NoticeAlertViewPresentationStyle)style
{
...
- (void)dismissWithStyle:(NoticeAlertViewDismissalStyle)style {
	self.dismissalStyle = style;
	[self performDismissalAnimation];
}

- (void)buttonWasPressed:(UIButton *)sender {
	NoticeAlertViewButtonBlock block = objc_getAssociatedObject(sender, kNoticeAlertViewButtonBlockKey);
	if(block) block();
...
{
	if(self.presentationStyle == NoticeAlertViewPresentationStylePop)
	{
	...
	else if(self.presentationStyle == NoticeAlertViewPresentationStyleFade)
	{
	...
	else
	{
        ...
        CGRect afterFrame = CGRectMake((CGRectGetWidth([UIScreen mainScreen].bounds) - 245) * 0.5, (CGRectGetHeight([UIScreen mainScreen].bounds) - 160) * 0.5, 245, 160);
...
- (void)performDismissalAnimation {
	NoticeAnimationCompletionBlock completionBlock = ^(BOOL finished)
	{
		[self.superview removeFromSuperview];
		[self removeFromSuperview];
	};
	
	if(self.dismissalStyle == NoticeAlertViewDismissalStyleTumble)
	{
	...
	else if(self.dismissalStyle == NoticeAlertViewDismissalStyleFade)
	{
	...
	else if(self.dismissalStyle == NoticeAlertViewDismissalStyleZoomDown)
	{
		[UIView animateWithDuration:0.3
							  delay:0.0
							options:UIViewAnimationOptionCurveEaseIn
	...
	else if(self.dismissalStyle == NoticeAlertViewDismissalStyleZoomOut)
	{
...
- (void)layoutSubviews {
	...
	CGRect cancelButtonRect = CGRectMake(boundingRect.origin.x, buttonOriginY, boundingRect.size.width * 0.5 - 4, NoticeAlertViewDefaultButtonHeight);
//	self.cancelButton.frame = cancelButtonRect;
	...
	if([self.otherButtons count] > 0)
	{
		...
		CGRect otherButtonRect = CGRectMake(cancelButtonRect.origin.x + cancelButtonRect.size.width + 8, buttonOriginY, boundingRect.size.width * 0.5 - 4, NoticeAlertViewDefaultButtonHeight);
		otherButton.frame = otherButtonRect;
        self.cancelButton.frame = cancelButtonRect;
	}else{
        self.cancelButton.frame = CGRectMake(boundingRect.origin.x, buttonOriginY, boundingRect.size.width , NoticeAlertViewDefaultButtonHeight);
    }
...
{
	CGRect rect = CGRectMake(0, 0, NoticeAlertViewDefaultWidth, NoticeAlertViewMinimumHeight);
											   ...
											   locationCount:(NSInteger)locationCount
{
	...
	CGRect rect = CGRectMake(0, 0, cornerRadius * 2 + 1, NoticeAlertViewDefaultButtonHeight);
