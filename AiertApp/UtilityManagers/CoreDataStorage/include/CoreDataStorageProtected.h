...
/**
 * The methods in this class are to be used ONLY by subclasses of CoreDataStorage.
**/

@interface CoreDataStorage (Protected)
...
/**
 * This is the standard configure method used by  extensions to configure a storage class.
 * 
 ...
 * 
 * Note that the default implementation allows the storage class to be used by multiple  streams.
...
/**
 * This class provides a caching service for Stream.myJID to avoid constantly querying the Stream for it.
 * 
 * The motivation behind this is the fact that to query the Stream for its myJID
 * requires going through the Stream's internal dispatch queue. (A dispatch_sync).
 ...
 * In addition, if we can stay out of Stream's internal dispatch queue,
 * we free it to perform more  processing tasks.
 * 
 * If the Stream.myJID changes, the cache will automatically be updated.
 * 
 * If you store any variant of Stream.myJID (bare, full, domain, etc) in your database
 ...
 * The data you're caching is explictly tied to the stream's bare myJID. ([Stream.myJID bare])
 * You want your storage class to support multiple  streams,
 * so you add a field to the database called streamBareJidStr (or whatever).
 * Given an Stream, you can use the built-in cache to quickly get the Stream.myJid property:
 * 
 * [self myJidForStream:stream]
 * 
 * This method will retrieve the myJID property of the given Stream the first time,
 * and then cache it for future lookups. The cache is automatically updated if the Stream.myJID ever changes.
**/
//- (JID *)myJIDForStream:(Stream *)stream;

/**
 * This method is invoked if the cached myJID changes for a particular  stream.
 * 
 * This method works in correlation with the myJIDForStream method.
 * In other words, calling myJIDForStream will cache the value.
 * If that value later changes, this method is invoked.
 * 
 * So if the myJID of an  stream changes, but there was no cached value for that  stream,
 ...
 * The default implementation does nothing.
**/
//- (void)didChangeCachedMyJID:(JID *)cachedMyJID forStream:(Stream *)stream;
 ...
 * from the parent Extension. Not only does this allow a single storage instance to service multiple extension
 * instances, but it provides the mechanism for the disk IO optimizations. The theory behind the optimizations
 * is to delay a save of the data (a slow operation) until the storage class is no longer being used. With 
 ...
 * 
 * This method is designed to be invoked from within the Extension storage protocol methods.
 ...
 * 
 * All of the methods that implement the Extension storage protocol invoke either executeBlock or scheduleBlock.
 * However, none of these methods invoke each other (they are only to be invoked from the Extension instance).
 * Instead, create internal utility methods that may be invoked.
 * 
 * For an example, see the RosterCoreDataStorage implementation's _userForJID:Stream: method.
**/
