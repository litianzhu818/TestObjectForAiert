...
- (NSManagedObjectModel *)managedObjectModel
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		LOG(@"%@: Creating managedObjectModel (%@)", [self class], momName);
		...
		{
			LOG(@"%@: Couldn't find managedObjectModel file - %@", [self class], momName);
		}
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		LOG(@"%@: Creating persistentStoreCoordinator", [self class]);
		...
		if (databaseFileName)
		{
			...
			{
				LOG(@"%@: Error creating persistentStoreCoordinator - Nil persistentStoreDirectory",
							[self class]);
			}
		}
		else
...
- (NSManagedObjectContext *)managedObjectContext
{
	...
	if (coordinator)
	{
		LOG(@"%@: Creating managedObjectContext", [self class]);
...
- (NSManagedObjectContext *)mainThreadManagedObjectContext
{
	...
	NSPersistentStoreCoordinator *coordinator = [self persistentStoreCoordinator];
	if (coordinator){
		LOG(@"%@: Creating mainThreadManagedObjectContext", [self class]);
...
- (void)managedObjectContextDidSave:(NSNotification *)notification
{
	...
	{
		LOG(@"Merging changes into mainThreadManagedObjectContext");
...
- (void)save
{
	...
	{
		LOG(@"%@: Error saving - %@ %@", [self class], error, [error userInfo]);
...
- (void)maybeSave:(int32_t)currentPendingRequests
{
	...
	if ([[self managedObjectContext] hasChanges])
	{
		...
		{
			LOG(@"%@: Triggering save (pendingRequests=%i)", [self class], currentPendingRequests);
		...
		else
		{
			...
			{
				LOG(@"%@: Triggering save (unsavedCount=%lu)", [self class], (unsigned long)unsavedCount);
