...
typedef struct zmodo_msg_hdr
{
    unsigned int magic; //0x0000ffff
    unsigned int version; //0x1
    unsigned int seqnum; //encrypt key
    unsigned int type; //0x4
    unsigned int length;
}zmodo_msg_hdr;//network byte order

/**
 * encrypt encoded zmodo sip message.
 *
 * @param src		The input buffer.
 * @param len		The buffer size(including zmodo headr).
 * @param dest   	the output buffer(including zmodo headr).
 */
void encrypt_zmodo_msg(char* src, int* len, char** dest);


/**
 * decrypt zmodo sip message.
 *
 * @param src		The input buffer.
 * @param len		The buffer size(excluding zmodo headr).
 * @param dest   	the output buffer(excluding zmodo headr).
 */
void decrypt_zmodo_msg(char* src, int* len, char** dest);
 ...
 * @return		PJ_SUCCESS if a message is found, or an error code.
 */
pj_status_t pjsip_find_zmodo_msg(const char *buf,
                                    pj_size_t size,
				    pj_bool_t is_datagram,
                                    pj_size_t *msg_size);
