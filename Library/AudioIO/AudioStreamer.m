...
{
    static  AudioStreamer *sharedIns = nil ;
...
- (void)startInternal
{
	...
	do
	{
//        DLog(@"_________________________________________________circle running : %d",self.state);
...
//
- (void)handleReadData:(NSData *)data
{
...
//    aspd[1].mDataByteSize = 1045;
//    aspd[1].mStartOffset = 0;
//    aspd[1].mVariableFramesInPacket = 0;
    
    [self handleAudioPackets:bytes numberBytes:length numberPackets:length*0.5 packetDescriptions:NULL];
//    if (discontinuous)
//    {
...
- (void)createQueue
{
    ...
    DLog(@"absd mSampleRate : %f",asbd.mSampleRate);
    DLog(@"absd mFormatID : %ld",asbd.mFormatID);
    DLog(@"absd mFormatFlags : %ld",asbd.mFormatFlags);
    DLog(@"absd mBytesPerPacket : %ld",asbd.mBytesPerPacket);
    DLog(@"absd mFramesPerPacket : %ld",asbd.mFramesPerPacket);
    DLog(@"absd mBytesPerFrame : %ld",asbd.mBytesPerFrame);
    DLog(@"absd mChannelsPerFrame : %ld",asbd.mChannelsPerFrame);
    DLog(@"absd mBitsPerChannel : %ld",asbd.mBitsPerChannel);
    DLog(@"absd mReserved : %ld",asbd.mReserved);

    DLog(@"dataOffset : %d",dataOffset);
...
//    DLog(@"handleAudioPacket!");
//    
//    DLog(@"inNumberBytes : %ld",inNumberBytes);
//    DLog(@"inNumberPackets : %ld",inNumberPackets);

//    DLog(@"mStartOffset : %lld",inPacketDescriptions->mStartOffset);
//    DLog(@"mVariableFramesInPacket : %ld",inPacketDescriptions->mVariableFramesInPacket);
//    DLog(@"mDataByteSize : %ld",inPacketDescriptions->mDataByteSize);
...
- (void)handleBufferCompleteForQueue:(AudioQueueRef)inAQ
	buffer:(AudioQueueBufferRef)inBuffer
{
//    DLog(@"handleBufferCompleteForQueue!");
...
#pragma mark - StopPlayReadData
+ (void)startPlayAudio
{
    dispatch_async(dispatch_get_main_queue(), ^{
        [[AudioStreamer sharedInstance] start];
    });
}
...
{
    dispatch_async(dispatch_get_main_queue(), ^{
        [[AudioStreamer sharedInstance] stop];
    });
}

#pragma mark - playReadData
+ (void)playAudioData:(NSData *)audioData
{
    [[AudioStreamer sharedInstance] performSelector:@selector(handleReadData:)
                                           onThread:[AudioStreamer sharedInstance]->internalThread
